...
class Creature:
    ...
    # Поліморфний метод, який перевизначатиметься в дочірніх класах
    def speak(self):
...
class User(Creature):
    ...
    def __init__(self, name, age, email, password, created_at):
        ...
        self.details = []  # Список для зберігання деталей
    ...
    # Метод для вибивання чека на касі
    def checkout(self):
        if not self.details:
    ...
    # Поліморфний метод speak, який перевизначає метод батьківського класу
    def speak(self):
...
class Employee(Creature):
    ...
    def __init__(self, name, age, email, password):
        ...
        self.__password = password
        self.__pincode = randint(100, 1000)
        self.orders_processed = 0  # Лічильник оброблених замовлень
        print("Your account has been created")
...
class Detail:
    ...
    def __init__(self, name, price):
        self.name = name
        self.price = price  # Ціна для кожної деталі

    def get_info(self):
        return f"Detail: {self.name}, Price: {self.price} USD"

    # Поліморфний метод speak
    def speak(self):
...
# Клас з однією ногою вилки (однонога вилка)
...
# Множинне наслідування: поєднує вилку з гарантією
...
class TwoLegsWithWarranty(TwoLegFork, Warranty):
    def __init__(self, name, length, year, width, material, leg_size, warranty_years, price):
        TwoLegFork.__init__(self, name, length, year, width, material, leg_size, price)  # Викликаємо конструктор TwoLegFork
        Warranty.__init__(self, warranty_years)  # Викликаємо конструктор Warranty
...
user.add_detail(frame)  # Додаємо раму
user.add_detail(one_leg_fork)  # Додаємо одноногу вилку
user.add_detail(two_leg_fork_with_warranty)  # Додаємо двоногу вилку з гарантією

# Перевірка всіх деталей, що додані користувачем
print("\nDetails added to the user:")
for detail in user.get_details():
    print(detail)

# Вибиваємо чек для користувача
print("\nCheckout process:")
user.checkout()
...
# Перевіряємо інформацію користувача
print("\nUser information:")
print(user.get_info())

# Викликаємо поліморфний метод speak() для користувача та деталей
print("\nSpeak demonstration:")
print(user.speak())
print(frame.speak())
...
# Створюємо працівника та демонструємо обробку замовлень
...
# Перевіряємо гарантію на двоногу вилку з гарантією
